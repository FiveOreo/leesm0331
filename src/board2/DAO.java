...
public class DAO {
	...
	public int count() {
		...
		try {
			sql = "SELECT COUNT(*) FROM MUSIC_Board2";
	...
	public ArrayList<VO> getMemberList() {
		...
		try {
			sql = "SELECT NUM, USERNAME, TITLE, TIME, HIT, INDENT from MUSIC_Board2 order by ref desc, step asc";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public int getMax() {
		...
		try {
			sql = "SELECT MAX(NUM) FROM MUSIC_Board2";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public void insertWrite(VO vo, int max) {
		...
		try {
			sql = "INSERT INTO MUSIC_Board2(NAME,PASS,TITLE,MEMO,FILENAME,REF) VALUES(?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
			...
			pstmt.setString(5, pasing(vo.getFilename()));
	...
	public VO getView(int idx) {
		...
		try {
			sql = "SELECT USERNAME, TITLE, MEMO, TIME, HIT, PASSWORD, REF, INDENT, STEP FROM MUSIC_Board2 WHERE NUM=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, idx);
			rs = pstmt.executeQuery();
	...
	public void UpdateHit(int idx) {
		...
		try {
			sql = "UPDATE MUSIC_Board2 SET HIT=HIT+1 WHERE NUM=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, idx);
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public boolean checkPassword(VO vo, int idx) {
		...
		try {
			sql = "SELECT NUM FROM Music_Board2 WHERE NUM=? AND PASSWORD=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, idx);
	...
	public void delete(int idx) {
		...
		try {
			sql = "DELETE FROM MUSIC_Board2 WHERE NUM=?";
			pstmt = con.prepareStatement(sql);
	...
	public void modify(VO vo, int idx) {
		...
		try {
			sql = "UPDATE MUSIC_Board2 SET TITLE=?, MEMO=?, where NUM=?";
			pstmt = con.prepareStatement(sql);
	...
	public void UpdateStep(int ref, int step) {
		...
		PreparedStatement pstmt = null;
		try {
			sql = "UPDATE MUSIC_Board2 SET STEP=STEP+1 where REF=? and STEP>?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, ref);
			pstmt.setInt(2, step);
			pstmt.executeUpdate();
	...
	public void insertReply(VO vo, int ref, int indent, int step) {
		...
		try {
			sql = "INSERT INTO MUSIC_Board2(USERNAME,PASSWORD,TITLE,MEMO,REF,INDENT,STEP,)" + "VALUES(?,?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
